...
- (void)viewDidLoad {
    ...
    /* 全局 navBar 默认隐藏（自绘标题），日志页临时显示 */
    [self.navigationController setNavigationBarHidden:NO animated:NO];

    self.tv = [UITextView new];
    self.tv.translatesAutoresizingMaskIntoConstraints = NO;
    self.tv.editable = NO;
    self.tv.font = [UIFont monospacedSystemFontOfSize:11 weight:UIFontWeightRegular];
    ...
    self.statusLabel.text = [NSString stringWithFormat:@"来源: %@", self.endpointURL ?: @""];
    [self reload];
}
...
- (void)reload {
    ...
    NSString *url = self.endpointURL ?: @"http://127.0.0.1:8080/log";
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:req completionHandler:^(NSData *data, NSURLResponse *resp, NSError *err) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (err) {
                self.tv.text = [NSString stringWithFormat:@"❌ 拉取失败\n\n%@\n\n可能原因:\n• 引擎未启动（请到「服务管理」点启动）\n• URL 不对（默认走 127.0.0.1:8080）", err.localizedDescription];
                return;
            }
            ...
            if (self.tv.text.length > 0) {
                NSRange r = NSMakeRange(self.tv.text.length - 1, 1);
